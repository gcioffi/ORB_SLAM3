...
void LoadImages(const string &strImagePath, const string &strPathTimes,
                vector<string> &vstrImages, vector<double> &vTimeStamps, int first_img);
...
int main(int argc, char *argv[])
{
    ...
    if(argc != 8)
    {
        cerr << endl << "Usage: ./mono_inertial_demo path_to_vocabulary path_to_settings path_to_image_folder path_to_times_file path_to_imu_file trajectory_file_name first_image" << endl;
        return 1;
    }
    ...
    string pathImu(argv[5]);
    int first_img = std::stoi(argv[7]);

    cout << "Loading images for sequence " << pathCam0 << "...";

    LoadImages(pathCam0, pathTimeStamps, vstrImageFilenames, vTimestampsCam, first_img);
    cout << "LOADED!" << endl;

    cout << "Loading IMU for sequence " << pathImu << "...";
    ...
    if(vTimestampsImu[0] > vTimestampsCam[0])
    {
        cerr << "ERROR: THe logic behind finding the first IMU measurement only work when firt IMU is older than first image" << endl;
        return 1;
    }
    ...
    std::string trajectory_file_name = argv[6];
